...
SELECT * 
FROM b_staging2
WHERE `Reporting Area` != LOCATION1 AND `Reporting Area` != LOCATION2;

-- Location1 and Location2 were repeats of reporting area --
ALTER TABLE b_staging2
DROP COLUMN LOCATION1,
DROP COLUMN LOCATION2;

CREATE TABLE b_staging3
LIKE b_staging2;
...
-- remaining null geocodes cannot be removed as they have important info on Maxs and flags --
...
-- Rearrange column order, drop sort_order as it is pointless information here --
ALTER TABLE b_staging3
...
CREATE TABLE botulism_cleaned AS
SELECT *
FROM b_staging3
ORDER BY area_id ASC, `Current MMWR Year` ASC, `MMWR WEEK` ASC;


SELECT * FROM nndss_dhhs_weekly_data.botulism_cleaned;

-- Prelim Check --

SELECT DISTINCT `Current week, flag`
FROM botulism_cleaned;
...
ALTER TABLE botulism_cleaned COMMENT = "U: Unavailable — The reporting jurisdiction was unable to send the data to CDC or CDC was unable to process the data.
